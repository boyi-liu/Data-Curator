"""Alpaca dataset -- prepare and load in one module.

As a library (the registry calls this): ``load(cfg, tokenizer)`` builds
``{data_dir}/alpaca.jsonl`` on first use -- the Stanford Alpaca 52k set
(Taori et al., 2023) reformatted to ``{"instruction", "input", "output"}`` --
then tokenizes it. Select with ``cfg.dataset.name = alpaca``.

As a script: ``python -m dataset.load_alpaca [--output ...] [--data_files ...]
[--sample_percentage ...]`` (re)builds just the JSONL.

The source defaults to the Hub (``tatsu-lab/alpaca``). Override via
``dataset.source`` (Hub id) or ``dataset.data_files`` (a local file), e.g.
``-o dataset.data_files=./raw/alpaca.json``.
"""
...
DEFAULT_SOURCE = "tatsu-lab/alpaca"
FILENAME = "alpaca.jsonl"
...
def prepare(cfg):
    """Build ``{data_dir}/alpaca.jsonl`` if missing; return its path."""
    ...
    if not os.path.exists(path):
        ...
        print(f"[alpaca] wrote {len(records)} examples to {path}")
    return path
...
def _cli():
    p = argparse.ArgumentParser(description="Build the Alpaca JSONL.")
