...
A naive, model-free selector that scores every training example by its lexical
...
DSIR): keep the training documents whose words best match the task we care
about.
...
The target/query set is the validation set when one is available; otherwise we
fall back to a random sample of the training set itself, so the score becomes a
self-similarity / centrality measure (documents that look like the rest of the
corpus rank highest). ``BaseSelector.topk_by_score`` then keeps the highest.

Operates purely on the raw prompt ``text`` field (see dataset/formatting.py), so
no model and no gradients are involved. Enable with ``--method bm25``.
"""
...
class Selector(BaseSelector):
    ...
    def select(self, train_dataset, val_dataset=None):
        ...
        return self.topk_by_score(agg.tolist())
